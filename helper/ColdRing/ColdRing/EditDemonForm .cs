...
namespace ColdRing
{
    public partial class EditDemonForm : Form
    {
        private List<Point> demons = new List<Point>();
        private DataGridView table = null;

        public EditDemonForm()
        {
            InitializeComponent();
        }

        public EditDemonForm(List<ColdRing.Point> demons, ref DataGridView table)
        {
            this.demons = demons;
            this.table = table;
            InitializeComponent();

            InputX.Text = table.SelectedRows[0].Cells[0].Value.ToString();
            InputY.Text = table.SelectedRows[0].Cells[1].Value.ToString();
            Text = string.Format("Изменение демона ({0}; {1})", InputX.Text, InputY.Text);
        }
        ...
        private void saved()
        {
            ...
            table.SelectedRows[0].Cells[0].Value = InputX.Text;
            table.SelectedRows[0].Cells[1].Value = InputY.Text;
            Close();
        }
